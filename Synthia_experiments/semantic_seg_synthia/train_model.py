...
TRAINVAL_DATASET = synthia_dataset.SegDataset(DATA, filelist_name='data_prep/trainval_raw.txt', npoints=NUM_POINT, num_frames=NUM_FRAME, train=True)
...
def train():
    with tf.Graph().as_default():
        ...
        log_string('Model parameters: %d' % count_params())
...
def get_batch(dataset, idxs, start_idx, end_idx, half=0):
    ...
    for i in range(bsize):
        pc, rgb, label, labelweights, loss_mask, _, _ = dataset.get(idxs[i+start_idx], half)
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for batch_idx in range(num_batches):
        for half in [0, 1]:
            ...
            summary, step, _, loss_val, _ = sess.run([ops['merged'], ops['step'],
