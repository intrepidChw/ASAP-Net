...
from common.pointnet2_lib.pointnet2.pointnet2_modules import PointnetSAModule, PointnetSAModuleMSG, PointnetFPModule
...
MLP1 = [[256, 128, 128, 256+128], [256, 128, 128, 256+128]]
FP_MLP1 = [512 + 256, 256]
RADIUS1 = [0.5, 1.0]
NPOINT1 = 4096
NSAMPLE1 = [16, 32]
...
class Backbone(nn.Module):
  ...
  def __init__(self, params):
    ...
    self.tem_sa1_msg = PointnetSAModuleMSG(mlps=MLP1, npoint=NPOINT1, nsamples=NSAMPLE1, radii=RADIUS1)
    self.fuse_layer1_atten = nn.Conv1d((MLP1[0][-1]+MLP1[1][-1])*2, 2, kernel_size=1, stride=1)
    self.fuse_layer1 = nn.Conv1d(MLP1[0][-1]+MLP1[1][-1], 512, kernel_size=1, stride=1)
  ...
  def forward(self, x):
    ...
    for frame_idx in range(frame_num):
        ...
        b, c, h, w = cur_x.shape
        cur_feat_l0 = cur_x.reshape(b, c, -1)
        # print('cur_xyz_l0:', cur_xyz_l0.shape)
        # print('cur_feat_l0:', cur_feat_l0.shape)

        if frame_idx == 0:
          cur_xyz_l1, cur_prim_feat_l1, sampled_feat_l1 = self.tem_sa1_msg(cur_xyz_l0.contiguous(), cur_feat_l0.contiguous())
          ...
          cur_xyz_l1, cur_prim_feat_l1, _ = self.tem_sa1_msg(expand_xyz_l0, expand_feat_l0, center_xyz_l1)
        ...
        cur_x = self.tem_fp1(cur_xyz_l0, cur_xyz_l1, cur_feat_l0, cur_feat_l1)
