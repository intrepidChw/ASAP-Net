...
class Segmentator(nn.Module):
  def __init__(self, ARCH, nclasses, path=None, path_append="", strict=False):
    ...
    if path is not None:
      ...
      try:
        ...
        backbone_keys = backbone_dict.keys()
        w_keys = w_dict.keys()
        for key in backbone_keys:
          if key in w_keys:
            backbone_dict[key] = w_dict[key]
  ...
  def forward(self, x, mask=None):
    ...
    # print('skips[0]:', skips[0].keys())
    y = self.decoder(y, skips)
    y_list = []
    for frame_idx in range(y.shape[1]):
      cur_y = y[:, frame_idx, ...]
      cur_y = self.head(cur_y)
      y_list.append(cur_y.unsqueeze(1))
    ...
    if self.CRF:
      ...
      for frame_idx in range(y.shape[1]):
        cur_x = x[:, frame_idx, ...]
        cur_y = y[:, frame_idx, ...]
        cur_mask = mask[:, frame_idx, ...]
        cur_y = self.CRF(cur_x, cur_y, cur_mask)
        y_list.append(cur_y.unsqueeze(1))
      y = torch.cat(y_list, dim=1)
    return y
