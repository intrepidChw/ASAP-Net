...
class SemanticKitti(Dataset):
  ...
  def __init__(self, root,    # directory where data is
    ...
    self.pred_root = os.path.join(pred_root, "sequences")
    ...
    for frame in frames:
        ...
        self.label_files.append(os.path.join(self.pred_root, frame_split[0], 'predictions', frame_split[1]+'.label'))
  ...
  def __getitem__(self, index):
    ...
    for idx in range(index*frame_num, index*frame_num+frame_num):
        ...
        if self.gt:
          ...
          scan.proj_sem_color[mask] = scan.sem_color_lut[scan.sem_label[scan.proj_idx[mask]]]
...
class Parser():
  ...
  def __init__(self,
    ...
    color_idx = [0, 10, 11, 15, 18, 20, 30, 31, 32, 40, 44, 48, 49, 50, 51, 70, 71, 72, 80, 81]
    new_color_map = {}
    for i in range(len(color_idx)):
        new_color_map[i] = self.color_map[color_idx[i]]
        # print('key: ', i, ' val: ', self.color_map[color_idx[i]])
    # Data loading code
